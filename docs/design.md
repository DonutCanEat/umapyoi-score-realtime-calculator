...
實測：`shots/live/` **15/15 全中**（6 張 1356→2560 全圖［含 2026-09-19 新收嘅 1929×1085］
＋ 4 張實機失敗／金色幀做回歸 ＋ 4 張實機狀態樣本 ＋ 1 張格線入墨窗口回歸
［`roi-theme-divider.png`，見地雷 #31］，信心 0.52–0.89）。
...
⚠️ **2026-09-19 再加一張「賽馬娘詳情」面板**（`shots/negatives/neg-uma-detail.png`，1931×1117）：
呢個畫面**同面板條一樣危險** —— 佢嘅面板橫向佔 0.13–0.44，啱啱好撞正 `roiX` 0.15–0.44，
而且有「能力值橫排」（1689/1349/1038/1302/1311 ＋ ランク徽章）。實測相對 ROI 內只搵到 4 條帶、
字元高 3px（預期 19px）→ `notBar`。⚠️ 呢張亦係**第一次有遊戲自己顯示嘅總分**可以對答案：
同一個面板顯示 **35,050 → UD1**，我哋查 `RANK_THRESHOLDS`（35,000 起 = UD1）**完全吻合**。
...
src/hud/layout.js   anchorHud()   相對位置 + 大細 + 偏移（全部 ÷ 內容區；見下面）
                    ...
                    hudState()    四態：ok／stale（讀唔到但保留上一個值）／none／edit（對位模式）
                                  同時組好顯示行：評價點、五維逐格、五維分、技能分、ランク目標
...
⭐ **C3 成長曲線**（2026-09-19 加）：`src/hud/history.js` 記住「五維／評價點隨時間變化」，
...
- **唔使等 Phase 2**：窗同 CLI 都係用「**而家五維**（讀得到就用實機，讀唔到就自己入數）＋
  ...
  ⚠️ 代價：五維係下限（技能分未讀到），所以顯示嘅「評價點」係 `≥` 而唔係實際總分。
...
冇併錯／漏併嘅跡象。⚠️ 門檻兩邊都貼（併入最低 **0.952**、未併最高 **0.949**）
