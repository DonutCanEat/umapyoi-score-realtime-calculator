...
| B4 | **收多啲數字樣本** 🔄 **做緊** | ① ✅ 由**現有** `shots/live-debug/` dump 收咗 **4 個新狀態**（264／387／220／1133）→ 實機面板條樣本 9 → **13**；⚠️ **每一格都用眼核實**（唔可以信舊 dump 嘅 `stats`：舊 build 嘅 `ok` 有假陽性，例如 `27/27/25/25/25`）。<br>② ✅ **2026-09-19 用戶實拍一張**（`live-1929x1085.png`，168/131/143/123/130，新解析度）→ 樣本 13 → **14**，重建模板後弱勢數字樣本上升（3 17→21、4 18→19、6 17→18、8 18→19、9 20→22）。<br>③ ✅ **2026-09-19 又收一張（⭐ 用戶報「讀唔到任何數據」嗰批幀）**：`roi-theme-divider.png`（216/86/123/105/143，人手放大核實）→ 樣本 14 → **15**。⚠️ 呢張唔係「又一張普通樣本」而係**新病種**：格與格之間嗰條半透明虛線落咗入墨色窗口（見地雷 #31）→ 所以佢同時係**永久回歸**（`diag-statbar --read` 15/15、`build-glyph-templates --verify` 15/15）。<br>④ ⏳ **要更多就要新實拍**（唔使改 code）：放一張**整個遊戲視窗**截圖（或者已剪 ROI）入 `shots/live/` ＋ 話我知五個數值 → 我用眼核實 → 入 `data/live-truth.json`（`shots` ＋ `perShot`）→ `node tools/build-glyph-templates.js --exclude=uma2`（驗證唔過**唔會**寫檔）。<br>⑤ 最弱三格：數字「6」20、「4」「8」20、「7」「9」22（全部都係同一隻馬／同一主題色 → 換馬換色會更有價值） | 中 |
| B5 | **其他畫面／狀態嘅面板條** 🔄 **做緊（閘已建）** | ① ✅ **搵到一個真 bug**：支援卡列表嘅 5 個 `Lv27` 徽章砌得出「5 個等距數字」→ 讀成 `27/27/25/25/25`（真值 700+）＝ **靜默報錯數** → 已修（兩條閘）＋ 入庫負樣本（見地雷 #30）。<br>② ✅ **負樣本閘**：`shots/negatives/`（資料夾就係宣告）—— `npm test`／`diag-statbar --read`／`build-glyph-templates --verify`／`replay-dumps` 四個閘都食。<br>③ ✅ **2026-09-19 用戶實拍兩張「培育結束確認」**（`neg-result-ability.png` 能力值 tab：最終五維 1689/1349/1038/1302/1311；`neg-result-skills.png` 技能 tab）→ 兩張都係**整個遊戲視窗**（1931×1117，內容框 top=31）→ 相對 ROI 落喺插畫 → `notBar` 唔出數 → 負樣本 3 → **5**。<br>④ ✅ **2026-09-19 再加「賽馬娘詳情」面板**（`neg-uma-detail.png`）→ 佢嘅面板橫向 0.13–0.44 **啱啱好撞正 `roiX` 0.15–0.44**，又有「能力值橫排」→ 實測字元高 3px（預期 19px）→ `notBar` → 負樣本 **6**。<br>⑤ ⏳ 未覆蓋：ステータス面板／比賽前後／訓練動畫／轉場 —— 有實拍就放入 `shots/negatives/`（唔准出數）或者 `shots/live/` ＋ 真值（要出數），即刻入閘。<br>⑥ ✅ **2026-09-23「培育結束確認 → 基礎能力」由負樣本升格做正面支援**（用戶要求「見到呢個畫面就讀基礎能力，算最後總分」）：`neg-result-ability.png` 搬去 `shots/result/result-ability-1931x1117.png` ＋ 入 `data/result-truth.json`；技能 tab 留喺負樣本。負樣本 6 → **5**，另加**第五個閘** `read-result.js --all` | 中 |
| B6 | **實機效能** | 5fps 下嘅延遲、CPU、記憶體（未正式量過） | 細 |
| B7 | ~~**核對最終評價点**~~ ✅ **已做（2026-09-19）—— 誤差 0** | 🔎 用戶提供「賽馬娘詳情」面板（`shots/negatives/neg-uma-detail.png` ＋ `shots/gt/uma5-detail-s{1,2}.png`）：評價点 **35,050**、ランク **UD1**、五維 1689/1349/1038/1302/1311。<br>　 ① ✅ **ランク表第一次同遊戲自己顯示嘅分對上**：35,050 落喺 `[35000,'UD1']`～`[35600,'UD2')` → UD1，同徽章一致。<br>　 ② ✅ **技能清單湊齊（27 招）之後，總分完全吻合**：五維 26,702 ＋ 固有 850（★4 Lv5）＋ 繼承固有 720（4 招）＋ 普通技能 6,778（22 招）＝ **35,050，差額 0**。→ `data/ground-truth/05-東海帝皇-超越地平線-UD1.json`，`fit-score` 由 **4/4 變 5/5、總誤差仍然 0**。<br>　 ③ ⭐ 意義：呢個係**第一次**用「遊戲自己顯示嘅總分」做真值（之前 4 條係同第三方紀錄對答案）→ ランク表、五維表、技能公式（含適性倍率、固有 ★×Lv、繼承固有 180）一次性全部驗到。<br>　 ④ ⚠️ 收集方法（捲動、綠色之前＝固有）記咗落 `data/ground-truth/README.md`「收樣本要點」 | 細 |
| B8 | ~~**`skill-overrides.json` 有 `base: null` 嘅條目**~~ ✅ **已澄清（用戶 2026-09-19）** | 用戶原話：「**綠色技能之前嘅技能都係固有技能** … 所以你嗰兩個 null 咗嘅技能都係固有」→ `VICTORY SHOT!`／`一鼓作氣♪Chu` 係**固有技能**（唔係有評價分嘅進化技能）→ 當「繼承固有 180」**係正確嘅**。✅ 實證：第 5 條樣本（啱啱好有 `VICTORY SHOT！`）湊齊清單之後**差額 0**。⚠️ `base: null` 嘅處理邏輯照舊（唔可以填 0，見 AGENTS 地雷 #4），但唔再係風險 | 細 |
...
| C4 | **訓練建議** | 🚧 **部分做（2026-09-19；未實機驗）**：`src/umascore/advice.js` ＋ `tools/advice.js` CLI ＋ what-if 窗「升級建議」一節 —— 做嘅係**屬性邊際效率**（`statPoints(v+1) − statPoints(v)`，差分唔係微分）＋「差 N 分約要加幾多點」（封頂屬性唔准入建議；跟建議加真係升到級有測試反證）。<br>　 ⭐ **2026-09-23（B 步）：訓練樣本引擎 ＋ CLI 已做** —— 「邊個訓練最好」嘅樽頸係**數據**（遊戲冇公開加成，而且跟支援卡組合／友情／幹勁變），所以**唔寫死任何表**，改為照**實測樣本**計：`src/umascore/training.js`（`gainsScore()` 用精確 `statPoints()` 差分算「呢筆加成值幾多評價分」；同一項多筆樣本出**中位數 ＋ min–max ＋ 樣本數**；**冇樣本 ＝ `known:false`，唔排名唔估**；技能Pt 另外列，唔混入評價分）＋ `tools/training.js` CLI（`--list`／`--add=speed:3:speed=12,power=6 --skill-pt=4`／`--remove=N`／`--stats=… --level=N`／`--json`）＋ `data/training-gains.json`（**空表入庫**，唔准放假數據）＋ `tools/lib/write-json.js`（原子寫 ＋ fsync，樣本係用家實測紀錄，寫壞就冇）＋ `test/training.test.js` **8 條**（精確差分／封頂／未知唔排名／等級唔准頂替／壞樣本點名 throw／出貨檔讀得入）。⏳ **未做**：(a) 收樣本（要玩，見下）；(b) 自動讀訓練畫面（C4-A 路線）；(c) what-if 窗顯示排名 | 大 |
...
| C8 | ~~**讀「培育結束確認」嘅最終五維**~~ ⏸️ **唔做（用戶 2026-09-19 決定：「暫時唔使」）** | 用戶原話：「第二張相係結算畫面…呢個亦都代表最終嘅五位數據」。決定理由：現階段**唔需要**自動讀呢個畫面 —— 守住「唔准亂出數」就夠（已入負樣本 `shots/negatives/neg-result-ability.png`／`neg-result-skills.png`），要算最終分就**手動輸入**（what-if 窗／`node src/cli.js 1689 1349 1038 1302 1311` → 26702 ＝ UF5）。⚠️ 保留將來要返嘅資料：能力值係**縱向清單**（屬性名 ＋ ランク徽章 ＋ 數值，非面板條）、金色係**空心輪廓**（唔可以混入現有實心模板）→ 要支援就係**新 reader path**，唔可以夾硬塞落 `statbar.js` | 中大 |
