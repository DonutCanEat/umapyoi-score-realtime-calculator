...
| B5 | **其他畫面／狀態嘅面板條** 🔄 **做緊（閘已建）** | ① ✅ **搵到一個真 bug**：支援卡列表嘅 5 個 `Lv27` 徽章砌得出「5 個等距數字」→ 讀成 `27/27/25/25/25`（真值 700+）＝ **靜默報錯數** → 已修（兩條閘）＋ 入庫負樣本（見地雷 #30）。<br>② ✅ **負樣本閘**：`shots/negatives/`（資料夾就係宣告）—— `npm test`／`diag-statbar --read`／`build-glyph-templates --verify`／`replay-dumps` 四個閘都食。<br>③ ✅ **2026-09-19 用戶實拍兩張「培育結束確認」**（`neg-result-ability.png` 能力值 tab：最終五維 1689/1349/1038/1302/1311；`neg-result-skills.png` 技能 tab）→ 兩張都係**整個遊戲視窗**（1931×1117，內容框 top=31）→ 相對 ROI 落喺插畫 → `notBar` 唔出數 → 負樣本 3 → **5**。<br>④ ✅ **2026-09-19 再加「賽馬娘詳情」面板**（`neg-uma-detail.png`）→ 佢嘅面板橫向 0.13–0.44 **啱啱好撞正 `roiX` 0.15–0.44**，又有「能力值橫排」→ 實測字元高 3px（預期 19px）→ `notBar` → 負樣本 **6**。<br>⑤ ⏳ 未覆蓋：ステータス面板／比賽前後／訓練動畫／轉場 —— 有實拍就放入 `shots/negatives/`（唔准出數）或者 `shots/live/` ＋ 真值（要出數），即刻入閘 | 中 |
| B6 | **實機效能** | 5fps 下嘅延遲、CPU、記憶體（未正式量過） | 細 |
| B7 | **核對最終評價点** 🔄 **部分做（2026-09-19）** | 🔎 **用戶提供嘅「賽馬娘詳情」面板（`shots/negatives/neg-uma-detail.png`）顯示：評價点 35,050、ランク UD1、五維 1689/1349/1038/1302/1311。**<br>　 ① ✅ **ランク表第一次同遊戲自己顯示嘅分對上**：35,050 落喺 `RANK_THRESHOLDS` 嘅 `[35000,'UD1']`～`[35600,'UD2')` → **UD1**，同遊戲徽章一模一樣。<br>　 ② ✅ 五維：我哋計 **26,702**（9225/5150/2832/4706/4789）→ 推得**技能分應該係 8,348**。<br>　 ③ ⚠️ **但係一屏技能清單解釋唔到**：用戶嗰張相嘅「技能」tab 一屏見到 14 招（絕對是我 Lv5、逐夢之星、VICTORY SHOT！、Shadow Break、∴win Q.E.D.、京都賽場○、喜歡獨處○、弧線的教授、彎道回復○、奔馳勝疾風、直線回復、集中力、取位天賦、尾段加速）。當中 9 招喺 DB（**全部「通用」→ 冇適性倍率**，合計 **2,308**）＋ 1 招固有（Lv5）＋ 2 招繼承固有（180 each）＋ 2 招 DB 冇（VICTORY SHOT！／Shadow Break）→ **遠遠唔夠 8,348** → 即係清單要**捲動**先睇得齊（GT01 就係「捲動前後兩張截圖合併去重」）。<br>　 ④ ⏳ **要做**：用戶喺「技能」tab **捲到底**再拍 1–2 張 → 湊齊技能清單 → 建 GT JSON（`node tools/fill-ground-truth.js` 自動填 base／條件／適性）→ `fit-score` 應該出**第 5 條樣本、誤差 0**（呢條係**第一次**用「遊戲自己顯示嘅總分」做真值，唔係第三方紀錄）。⚠️ 技能 Pt 1325 **唔等於**技能分 | 細 |
| B8 | **`skill-overrides.json` 有 `base: null` 嘅條目**（計分風險） | `VICTORY SHOT!`（Pt 0）同 `一鼓作氣♪Chu`（Pt 0）嘅 base 係 **null** → `fill-ground-truth.js` 會**跳過**佢哋 → 之後 lookup 唔到 → **被當成「繼承固有 180」**。如果佢哋真係劇本進化技能而有真實評價分（可以係幾百），GT 就會少計。⚠️ 4 條現有樣本嘅誤差仍然係 0（所以未爆），但 B7 嗰條新紀錄啱啱好有 `VICTORY SHOT!` → 湊齊清單之後**第一件事就係驗呢個**：如果總分對唔上，差額大概就係呢兩招。 | 細 |
