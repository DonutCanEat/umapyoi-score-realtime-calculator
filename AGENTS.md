...
npm.cmd test              # 單元測試（197 個，必須全過；⭐ 乾淨 checkout 一樣要全過 —— 見 §8）
...
**驗收標準**（全部都要）：
1. `npm.cmd test` 全過（現時 **197 個**；⭐ 乾淨 `git archive HEAD` checkout 一樣要全過）
...
1. `npm.cmd test`（或 `node --test --test-isolation=none test/*.test.js`）— **197 個測試必須全過**
