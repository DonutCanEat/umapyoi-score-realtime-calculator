...
| Phase 1 | **五維數字辨識（零校準）** | ✅ 兩條路都通：**畫面 A 面板條**（`statbar.js`）**9/9 全中**（1356→2560 五個解析度 ＋ 4 個實機失敗／金色幀回歸）；ステータス面板排法 **30/30**。✅ 已實機跑過（`npm start`，1920 窗），修好間歇性「讀唔清」（地雷 #25）同**金色高亮靜默讀錯**（地雷 #26）|
...
>
> **第三次實機跑（60 幀 dump）：金色高亮 → 靜默讀錯（地雷 #26）**。用戶報
> 「1489 讀成 1483、1613 讀成 1513」→ 查真幀證實：屬性升咗之後數字變**金色**，
> 淺金高光（亮度 0.98）過唔到墨色窗口 → 字形被侵蝕（每個字元相似度只剩 0.46–0.60）
> → 打和／揀錯。**正解：偵測到金色就唔出數**（墨點色相 p90 ≥ 33°，正常幀 27–28°）
> → 金幀入 `shots/live/roi-regress-gold.png` 做永久回歸（判準＝「唔准出數」）。
...
| 26 | ⭐⭐ **以為「所有狀態嘅數字都係橙棕色」** | ⛔ **最危險嘅一類：靜默讀錯數**（用戶實機報 1489→**1483**、1613→**1513**）。屬性升咗之後，遊戲會把該格數字畫成**金色**：深金邊 ＋ **極淺金高光**（實測 `rgb(255,255,214)`、色相 60°、**亮度 0.98**）。橙棕窗口（lum < 0.62）會削走淺金部分 → 字形被侵蝕（「9」上圈左邊整條唔見）→ **每個字元相似度只剩 0.46–0.60**（正常 0.8–0.9）→ 打和／揀錯（實測 3:0.60 vs 9:0.60）。⚠️ **試過加「淺金窗口」反而更差**（8 讀成 5）。**正解**：偵測到金色高亮就**唔出數** —— 判準＝墨點色相 **p90**（實測正常幀 27.2–27.8°、金幀 37.5–38.7° → 門檻 **33**）→ 回 `highlighted: true`，HUD 沿用上一個穩定值。永久回歸：`shots/live/roi-regress-gold.png` ＋ `data/live-truth.json` 嘅 `expectHighlighted`（判準係「唔准出數」）|
...
⚠️ **金色高亮**（屬性升咗之後）：字形被侵蝕 → **唔出數**（`highlighted: true`），
判準係墨點色相 p90 ≥ 33°（見地雷 #26）。所以「唔出數」有三種，HUD 要分得清：
`notBar`（換咗畫面）、`highlighted`（金色，等佢回復）、真失敗（要查）。
...
| 優先 | 事項 |
|---|---|
| ⭐ 高 | **搞清楚金色高亮會持續幾久**（地雷 #26 嘅後續）：現時策略係「金色 → 唔出數，沿用上一個穩定值」。如果金色只係升屬性後幾秒（好可能），咁就冇問題；但如果**金色會持續好耐**（例如直到下次訓練），就要另想辦法（例如為金色另建一套模板，或者用幾何／結構特徵而唔靠墨色）—— 需要用戶實機觀察確認 |
| ⭐ 高 | **HUD overlay ＋ 設定面板**（下一步）：`readStatBar()` 已經實機讀到五維並算好評價分（9/9 回歸全中），但仲只係 `console.log`。要開透明置頂穿透視窗顯示，技能未讀到就老實顯示 `技能分 ？／總分 ≥ X`；「唔見面板條」／「金色」時**保留上一個穩定值**而唔係閃走（`StatTracker` 已經有 `current`，接去 HUD 就得）|
