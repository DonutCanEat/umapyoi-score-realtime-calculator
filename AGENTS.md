...
npm.cmd test              # 單元測試（303 個，必須全過；⭐ 乾淨 checkout 一樣要全過 —— 見 §8）
...
**驗收標準**（全部都要）：
1. `npm.cmd test` 全過（現時 **303 個**；⭐ 乾淨 `git archive HEAD` checkout 一樣要全過）
...
test/           # 303 條（`npm.cmd test`）—— 純函數 ＋ 幾個**接線閘**（static wiring gate）
...
1. `npm.cmd test`（或 `node --test --test-isolation=none test/*.test.js`）— **303 個測試必須全過**
   ...
   有 `hud-position.json`），修好之後兩邊一樣；而家兩邊都係 **303／0**）。
