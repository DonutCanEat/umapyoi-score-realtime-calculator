...
| Phase 2 | 技能 icon 識別（自動知學咗邊啲技能）| 🚧 **第一步做好**：技能畫面欄／行偵測器（`skillscreen.js`，8 張實機圖全部搵到 7 行）。⏳ 未做：識字（見 §9 同 `docs/skill-screen.md` §4 嘅**硬限制**）|
...
| 中 | **Phase 2 下一步：技能畫面「名稱框」抽取**（`skillscreen.js` 已偵測到 7 行／左右兩欄）：下一步係由每欄切出「技能名」區域（剔走 icon 同右邊 Lv），量字元切分（要處理 `◎ ○ × ・ ∴` 等符號）。⚠️ **識字有硬限制**：冇遊戲字型檔、冇 1300 招標註樣本 → 唔可以承諾全自動讀名；可行路線係「名稱影像比對候選名單」（見 `docs/skill-screen.md` §4），要先同用戶對清楚**候選名單邊度嚟**（用家輸入？按適性篩？）|
