...
test/           # 267 條（`npm.cmd test`）—— 純函數 ＋ 幾個**接線閘**（static wiring gate）
...
1. `npm.cmd test`（或 `node --test --test-isolation=none test/*.test.js`）— **267 個測試必須全過**
   ...
   有 `hud-position.json`），修好之後兩邊一樣；而家兩邊都係 **267／0**）。
