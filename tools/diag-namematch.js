 ...
 * ## 點解要呢個工具
 *
 * 之前幾輪都係「揾真值去驗」：人手標註錯（我錯位一行）、幾何自標註又錯
 * （同一招喺唔同頁會落喺唔同位置 → 「同位置 = 同一招」唔成立，cross-group 都出現 1.000）。
 * 結論：**冇一個免費嘅真值**。
 *
 * 所以改用**唔需要真值嘅檢定**：
 *
 * 1. **互相最佳配對（mutual best match）**：對每個名框，喺其他圖揾最似嗰個。
...
/** 框內墨跡 → 絕對尺度特徵（1 像素 = 1 格、上下居中；見 `diag-namepairs.js` 註釋）。 */
...
for (let i = 0; i < all.length; i += 1) {
  for (let j = i + 1; j < all.length; j += 1) {
    ...
    aligned.push({ s: dot(a.vec, b.vec), a, b });
  }
}
